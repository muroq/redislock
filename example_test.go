...
func Example() {
	...
	// Try to obtain lock.
	lock, err := locker.Obtain(ctx, "my-key", 100*time.Millisecond, 100*time.Millisecond, nil)
...
func ExampleClient_Obtain_retry() {
	...
	lock, err := locker.Obtain(ctx, "my-key", time.Second, time.Second, &redislock.Options{
